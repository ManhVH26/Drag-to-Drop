...
import { NormalizedScreen, ScreenObject } from "@/lib/dynamic-ob/types";
import { SCREEN_TYPE_COLORS } from "@/lib/dynamic-ob/constants";
...
export default function SortableScreenCard({
  screen,
  index,
  isRequired,
  onEdit,
  ...
  const obj = screen.isObject ? (screen.raw as ScreenObject) : null;
  const title = obj?.title;
  const subtitle = obj?.subtitle;
  const canEdit = screen.isObject;
    ...
    >
      ...
      <span
        className={`px-2 py-0.5 rounded text-xs font-medium whitespace-nowrap ${
          SCREEN_TYPE_COLORS[screen.type] || "bg-gray-100 text-gray-600"
        }`}
      >
        {screen.type}
      </span>
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
        ...
        <button
          ...
          title={isRequired ? "Cannot delete required screen" : "Delete"}
        >
          <svg
            width="16"
            height="16"
